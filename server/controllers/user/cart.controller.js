...
const addToCart = async (req, res) => {
    ...
    try {
        ...
        if (!Exist_cart) {
            ...
            newCart.save();
            const CartItems = await fetchCartWithProductDetails(userId)
            return res.status(200).json({ success: true, message: "New Cart created", CartItems })
        }
        ...
        Exist_cart?.save();
        const CartItems = await fetchCartWithProductDetails(userId)
        return res.status(200).json({ success: true, message: 'product Added To Cart', CartItems })
...
const deleteItemFromCart = async (req, res) => {
    ...
    try {
        ...
        const filterProduct = Exist_cart.items.filter((item) => item.productId.toString() !== productId);
        Exist_cart.items = filterProduct;
        await Exist_cart.save()
        const CartItems = await fetchCartWithProductDetails(userId)
        return res.status(200).json({ success: true, message: 'Product is successfuly Removed', CartItems })
    } catch (error) {
        console.log(`error in addToCart  ${error.message}`);
        res.status(500).json({ success: false, message: `server side error error is ${error.message}  you can Report this Error` })
    }
