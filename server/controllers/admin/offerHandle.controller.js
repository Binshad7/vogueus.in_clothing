...
const createOffer = async (req, res) => {
    ...
    try {
        ...
        if (productId) {
            ...
            if (!isScheduled) {
                updateProductCurrentPrice(product);
            }
    ...
    try {
        ...
        for (const product of products) {
            ...
            if (product.offers && product.offers.length > 0) {
                ...
                console.log('activeProductOffers :', activeProductOffers)
            ...
            if (shouldApply) {
                ...
                const productUpdating = await Product.updateOne({ _id: product._id }, { $set: { currentPrice: finalAmount } })
            }
        }
    } catch (error) {
...
// Helper function to update a product's currentPrice based on best active offer
async function updateProductCurrentPrice(product) {
    ...
    if (activeOffers.length > 0) {
        // Find best discount among active offers
        ...
        // No active offers
        product.currentPrice = 0; // Reset to 0 as per your requirement
    }
    await product.save();
}

// Setup Cron Jobs for Offer Management
...
// Activate scheduled offers that should now be active
...
async function expireOldOffers(currentDate) {
    try {
        ...
        for (const product of productsWithExpiredOffers) {
            ...
            // Update currentPrice based on remaining offers
            updateProductCurrentPrice(product);
            await product.save();
        }
...
const getActiveOffers = async (req, res) => {
    try {
        ...
        }).select('productName regularPrice currentPrice offers');

        // Get active subcategoryId offers
        const subcategoriesWithOffers = await subcategory.find({
            "offers": {
                $elemMatch: {
                    startDate: { $lte: currentDate },
                    endDate: { $gt: currentDate }
        ...
        }).select('subcategoryIdName offers');

        return res.status(200).json({
            productOffers: productsWithOffers,
            subcategoryIdOffers: subcategoriesWithOffers,
            totalActiveOffers: productsWithOffers.length + subcategoriesWithOffers.length
        });
    } catch (error) {
...
const getScheduledOffers = async (req, res) => {
    try {
        const currentDate = new Date();

        // Get scheduled product offers
        const productsWithScheduledOffers = await Product.find({
            "offers": {
                $elemMatch: {
                    startDate: { $gt: currentDate }
                }
            }
        }).select('productName regularPrice offers');

        // Get scheduled subcategoryId offers
        const subcategoriesWithScheduledOffers = await subcategory.find({
            "offers": {
                $elemMatch: {
                    startDate: { $gt: currentDate }
                }
        ...
        return res.status(200).json({
            scheduledProductOffers: productsWithScheduledOffers,
            scheduledsubcategoryIdOffers: subcategoriesWithScheduledOffers,
            totalScheduledOffers: productsWithScheduledOffers.length + subcategoriesWithScheduledOffers.length
        });
    } catch (error) {
